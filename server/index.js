...
app.get("/my-lessons", verifyJWT, async (req, res) => {
  ...
  const lessons = await lessonsCollection
    .find({ authorEmail: email, existStatus: "active", isDeleted: false })
    .sort({ createdAt: -1 })
    .toArray();
...
// soft-delete lesson for user/admin
app.delete("/lessons/:id", verifyJWT, async (req, res) => {
  const id = new ObjectId(req.params.id);
  const deletedBy = req.tokenEmail;

  const result = await lessonsCollection.updateOne(
    { _id: id, existStatus: "active" },
    {
      $set: {
        existStatus: "trash",
        isDeleted: true,
  ...
  if (result.modifiedCount === 0) {
    return res
      .status(404)
      .send({ message: "Lesson not found or already deleted." });
  }

  await Promise.all([
    favoriteCollection.deleteMany({ lessonId: id }),
    commentsCollection.deleteMany({ lessonId: id }),
  ]);

  res.send({ success: true, message: "Lesson moved to trash." });
});
...
// get lessons in trash for admin
app.get("/admin/lessons/trash", verifyJWT, verifyADMIN, async (req, res) => {
  try {
    const { page = 1, limit = 10 } = req.query;
    const skip = (page - 1) * limit;

    const [trashLessons, total] = await Promise.all([
      lessonsCollection
        .find({ existStatus: "trash", isDeleted: true })
        .sort({ deletedAt: -1 })
        .skip(Number(skip))
        .limit(Number(limit))
        .toArray(),
    ...
    res.send({
      lessons: trashLessons,
      totalPages: Math.ceil(total / limit),
      totalLessons: total,
    });
  } catch (error) {
    console.error("Error fetching trash lessons:", error);
    res.status(500).send({ message: "Internal server error" });
  }
});
...
// soft delete a lesson and move it to trash
app.patch(
  "/admin/report/soft-delete/:id",
  verifyJWT,
  verifyADMIN,
  async (req, res) => {
    try {
      ...
      const result = await lessonsCollection.updateOne(
        { _id: lessonId, existStatus: "active" },
        {
          $set: {
            existStatus: "trash",
            isDeleted: true,
      ...
      if (result.modifiedCount === 0) {
        return res
          .status(404)
          .send({ success: false, message: "Active lesson not found." });
      }

      await Promise.all([
        favoriteCollection.deleteMany({ lessonId }),
        commentsCollection.deleteMany({ lessonId }),
      ]);

      res.send({ success: true, message: "Lesson moved to Trash Bin." });
    } catch (error) {
...
app.patch(
  ...
  async (req, res) => {
    try {
      ...
      const result = await lessonsCollection.updateOne(
        { _id: lessonId, existStatus: "trash" },
        {
          $set: {
            existStatus: "active",
            isDeleted: false,
      ...
      if (result.modifiedCount === 0) {
        return res
          .status(404)
          .send({ success: false, message: "Lesson not found in trash." });
      }

      res.send({ success: true, message: "Lesson restored successfully." });
    } catch (error) {
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (!status) {
        return res
          .status(400)
          .send({ success: false, message: "Status parameter is required" });
      }
      ...
      res.send({
        success: true,
        message: "Reports handled and lesson status reset successfully.",
      });
    } catch (error) {
...
app.get("/admin/profile/stats", verifyJWT, verifyADMIN, async (req, res) => {
  const email = req.tokenEmail;
